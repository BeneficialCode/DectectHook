...
ULONG PEParser::GetExportByName(PCSTR exportName) {
	...
	auto data = static_cast<IMAGE_EXPORT_DIRECTORY*>(RVA2FA(dir->VirtualAddress));
	auto ordinals = (PBYTE)(data->AddressOfNameOrdinals != 0 ? RVA2FA(data->AddressOfNameOrdinals) : nullptr);
	auto names = (PUCHAR)(data->AddressOfNames != 0 ? RVA2FA(data->AddressOfNames) : nullptr);
	auto functions = (PULONG)RVA2FA(data->AddressOfFunctions);
	auto ordinalBase = data->Base;
	PCSTR name = nullptr;
